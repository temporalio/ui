...
export const ListSearchAttributesResponse = z.object({
  ...
  systemAttributes: z
    ...
    .describe('Mapping between system (predefined) search attribute name to its IndexedValueType.')
    .optional(),
  /**Mapping from the attribute name to the visibility storage native type.*/
  storageSchema: z
    .record(z.string())
    .describe('Mapping from the attribute name to the visibility storage native type.')
    .optional(),
});
export type ListSearchAttributesResponse = z.infer<typeof ListSearchAttributesResponse>;
