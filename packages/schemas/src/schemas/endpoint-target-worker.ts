...
export const EndpointTarget_Worker = z
  .object({
    ...
    namespace: z.string().describe('Namespace to route requests to.').optional(),
    /**Nexus task queue to route requests to.*/
    taskQueue: z.string().describe('Nexus task queue to route requests to.').optional(),
  })
  .describe('Target a worker polling on a Nexus task queue in a specific namespace.');
