...
export const WorkflowExecutionFailedEventAttributes = z.object({
  ...
  failure: z
    .any()
    .describe('Serialized result of workflow failure (ex: An exception thrown, or error returned)')
    .optional(),
  retryState: z
    .enum([
  ...
  newExecutionRunId: z
    .string()
    .describe('If another run is started by cron or retry, this contains the new run id.')
    .optional(),
});
