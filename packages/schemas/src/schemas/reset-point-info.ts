...
export const ResetPointInfo = z
  .object({
    ...
    runId: z
      .string()
      .describe('The first run ID in the execution chain that was touched by this worker build.')
      .optional(),
    /**Event ID of the first WorkflowTaskCompleted event processed by this worker build.*/
    firstWorkflowTaskCompletedId: z
      .string()
      .describe('Event ID of the first WorkflowTaskCompleted event processed by this worker build.')
      .optional(),
    ...
    resettable: z
      .boolean()
      .describe('false if the reset point has pending childWFs/reqCancels/signalExternals.')
      .optional(),
  })
  .describe(
    'ResetPointInfo records the workflow event id that is the first one processed by a given\n build id or binary checksum. A new reset point will be created if either build id or binary\n checksum changes (although in general only one or the other will be used at a time).',
