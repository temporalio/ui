...
export const RegisterNamespaceRequest = z.object({
  ...
  data: z.record(z.string()).describe('A key-value map for any customized purpose.').optional(),
  securityToken: z.string().optional(),
  isGlobalNamespace: z.boolean().optional(),
  /**If unspecified (ARCHIVAL_STATE_UNSPECIFIED) then default server configuration is used.*/
  historyArchivalState: z
    .enum(['ARCHIVAL_STATE_UNSPECIFIED', 'ARCHIVAL_STATE_DISABLED', 'ARCHIVAL_STATE_ENABLED'])
    .describe(
      'If unspecified (ARCHIVAL_STATE_UNSPECIFIED) then default server configuration is used.',
    )
    .optional(),
