...
export const NamespaceInfo_Capabilities = z
  .object({
    ...
    syncUpdate: z.boolean().describe('True if the namespace supports sync update').optional(),
    /**True if the namespace supports async update*/
    asyncUpdate: z.boolean().describe('True if the namespace supports async update').optional(),
  })
  .describe(
    'Namespace capability details. Should contain what features are enabled in a namespace.',
  );
export type NamespaceInfo_Capabilities = z.infer<typeof NamespaceInfo_Capabilities>;
