...
export const BuildIdReachability = z
  .object({
    ...
    buildId: z.string().describe('A build id or empty if unversioned.').optional(),
    /**Reachability per task queue.*/
    taskQueueReachability: z.array(z.any()).describe('Reachability per task queue.').optional(),
  })
  .describe('Reachability of tasks for a worker by build id, in one or more task queues.');
