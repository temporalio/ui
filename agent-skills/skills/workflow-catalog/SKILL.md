...
The Workflow Catalog is a local-development page in `temporalio/ui` for browsing and running Temporal workflow examples. Examples are declared in code, generated into artifacts the browser reads, and executed by a worker this repository starts.
...
`pnpm dev` is unaffected by the catalog and never starts a worker. To run the pieces separately, use `pnpm dev` in one terminal and `pnpm workflow-catalog worker` in another.
...
```bash
pnpm workflow-catalog help                     # list commands
pnpm workflow-catalog scaffold <example-id>    # create a local example
...
- **OSS** — shared examples committed to this repository, under `src/lib/workflow-catalog/worker/examples/<id>/`.
- **Local** — your own examples, under `workflow-catalog.local/examples/<id>/`. That directory is gitignored and never ships.

The generated `src/lib/workflow-catalog/browser/catalog.generated.json` lists every shared example with its capability tags, expected evidence, input schema, and any setup instructions. Read it to answer "what examples exist" without starting anything.
...
pnpm workflow-catalog promote order-lifecycle
```
...
- **Namespace** comes from where you are. In the browser it is the namespace in the URL, so browsing `/namespaces/staging/workflow-catalog` targets `staging`. For the worker it comes from `TEMPORAL_NAMESPACE`. Point both at the same namespace or the UI will show no worker polling.
...
| ------------------------------------- | --------- | ---------------------------------------------------- |
| `.env.workflow-catalog`               | yes       | Address and namespace defaults for local development |
| `.env.workflow-catalog.local`         | no        | Your overrides and all credentials                   |
| `.env.workflow-catalog.local.example` | yes       | Commented template for every credential mode         |
...
`catalog.generated.json`, `catalog.generated.ts`, and the local assemblies are generated. Never edit them by hand; run `pnpm workflow-catalog generate`. Verification compares a hash of the declared source files, so a stale artifact fails with a message naming the command that fixes it. `pnpm workflow-catalog verify` is the check to run before committing catalog changes.
