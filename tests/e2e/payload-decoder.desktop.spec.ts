...
const PAYLOAD_COVERAGE_WORKFLOW_ID = 'payload-coverage-workflow';
const USER_METADATA_WORKFLOW_ID = 'user-metadata-workflow';

test.describe('Payload Decoder', () => {
  test.describe('Payload Coverage Workflow', () => {
    test.beforeEach(async ({ page, baseURL }) => {
      await page.goto(baseURL);
      await page
        .getByRole('link', { name: PAYLOAD_COVERAGE_WORKFLOW_ID, exact: true })
        .click();
    });
    ...
    test('workflow input is decoded', async ({ page }) => {
      ...
      await expect(workflowDetail.inputAndResult).toContainText(
        'arrayOfStrings',
      );
    });
    ...
    test('event history StartChildWorkflowExecutionInitiated shows decoded input', async ({
      ...
      await workflowDetail.openHistory();
      await eventHistory.switchToFeed();
      await eventHistory.expandEvent(
    ...
    test('send signal and verify decoded signal payload in history', async ({
      ...
      await signalModal.fillPayload(
        '{"key":"testKey","value":{"nested":true}}',
      );
  ...
  test.describe('User Metadata Workflow', () => {
    test.beforeEach(async ({ page, baseURL }) => {
      await page.goto(baseURL);
      await page
        .getByRole('link', { name: USER_METADATA_WORKFLOW_ID, exact: true })
        .click();
    });
