export default async function callLLMGemini(
  ...
  return {
    ...
    _details: {
      ...
      response: `Here's a comparison of orchestration approaches for "${prompt}":\n\nTemporal: Full replay-based recovery, event history + UI, medium complexity\nLangGraph: Checkpoint-based, LangSmith traces, low complexity\nCustom (Redis + queues): Manual implementation, custom logging, high complexity\n\nRecommendation: For production AI agents, Temporal provides the strongest durability guarantees. LangGraph is easier to start with but hits scaling limits in production.`,
      promptTokens: 175,
