...
import { fetchAllSchedules } from '../services/schedule-service';
import PageTitle from '../holocene/page-title.svelte';
import NamespaceSelector from '../holocene/namespace-selector.svelte';
import SchedulesTable from '../components/schedule/schedules-table.svelte';
...
let search = '';
$: filteredSchedules = (schedules) => search
    ? schedules.filter((schedule) => schedule.scheduleId.toLowerCase().includes(search.toLowerCase()))
    : schedules;
const errorMessage = 'Create scheduled actions using our Public API or TCTL (CLI).';
</script>
