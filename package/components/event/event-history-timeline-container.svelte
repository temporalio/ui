...
import { ascendingEventGroups, ascendingEvents } from '../../stores/events';
import { workflowRun, refresh } from '../../stores/workflow-run';
import { timelineEvents } from '../../stores/events';
import Accordion from '../../holocene/accordion.svelte';
import Button from '../../holocene/button.svelte';
import Icon from '$holocene/icon/icon.svelte';
import { eventTypeCategorizations, timelineEventTypeOptions, } from '../../models/event-history/get-event-categorization';
import EventHistoryGroupTimeline from './event-history-group-timeline.svelte';
import Autocomplete from '../../holocene/autocomplete.svelte';
...
let eventGroupFilters = timelineEventTypeOptions.map((o) => o.option);
const handleInput = (e) => {
...
$: groups = !eventTypeFilters.length
    ...
    : groupsWithGroupFilter.filter((group) => {
        return group.eventList.find((event) => eventTypeFilters.includes(event.eventType));
    });
</script>
