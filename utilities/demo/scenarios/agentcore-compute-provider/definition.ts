...
export const definition = defineScenario({
  ...
  preview: {
    notes: [
      ...
      'Open Storybook\'s "Self-hosted (every provider selectable)" story. All three providers must be selectable and nothing may read "Coming Soon". This is the behaviour change: a self-hosted server previously gated providers on capabilities it never advertised, which left Cloud Run permanently unselectable and AgentCore badged Coming Soon forever.',
      'Open "Locked to the provider a Version uses". Only AgentCore may be shown, and it must be selectable rather than greyed. A Version\'s provider cannot be changed, so offering alternatives that cannot be applied is a worse lie than hiding them, and a card that is selected and disabled at the same time reads as broken.',
