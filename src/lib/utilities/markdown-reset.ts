...
/* A label on one line, whatever the markdown does. A summary written as two
   paragraphs, or with a hard break, renders as separate blocks: the frame is
   sized from that content, so the label paints as two stacked lines in a row
   that has height for one. Inline runs the blocks together instead, and the
   text is measured and painted as the single line the row expects. Inline is
