...
<Hst.Story>
  <Hst.Variant title="With Radio Menu Items">
    ...
    >
      <RadioMenuItem
        checked={value === 7}
        value={7}
        on:select={(event) => hstEvent('select', event)}>7 Days</RadioMenuItem
      >
      <RadioMenuItem
        checked={value === 30}
        value={30}
        on:select={(event) => hstEvent('select', event)}>30 Days</RadioMenuItem
      >
      <RadioMenuItem
        checked={value === 60}
        value={60}
        on:select={(event) => hstEvent('select', event)}>60 Days</RadioMenuItem
      >
      <RadioMenuItem
        checked={value === 90}
        value={90}
        on:select={(event) => hstEvent('select', event)}>90 Days</RadioMenuItem
      >
    </DropdownButton>
  </Hst.Variant>
  ...
  <Hst.Variant title="With Regular Menu Items">
    <DropdownButton id="dropdown-button-2" label="Regions" icon="regions">
      <MenuItem on:click={(event) => hstEvent('click', event)}
        >us-east-1</MenuItem
      >
      <MenuItem on:click={(event) => hstEvent('click', event)}
        >us-east-2</MenuItem
