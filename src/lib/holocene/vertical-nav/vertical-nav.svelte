...
>
  ...
  <div
    ...
    class={`pointer-events-none absolute left-0 w-full bg-io-actions-press-overlay transition-all ${ACTIVE_TRANSITION_DURATION} ease-out`}
    aria-hidden="true"
  ></div>
  ...
  {#if !hideHoverBackground && $hoveredItem && $hoveredItem !== $activeItem}
    <div
      ...
      class={`pointer-events-none absolute left-0 w-full bg-io-actions-hover-overlay ${
        enableTransitions
