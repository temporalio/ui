...
>
  ...
  <div
    ...
    class={`pointer-events-none absolute left-0 w-full bg-action-press-overlay transition-all ${ACTIVE_TRANSITION_DURATION} ease-out`}
    aria-hidden="true"
  ></div>
  ...
  {#if !hideHoverBackground && $hoveredItem && $hoveredItem !== $activeItem}
    <div
      ...
      class={`pointer-events-none absolute left-0 w-full bg-action-hover-overlay ${
        enableTransitions
