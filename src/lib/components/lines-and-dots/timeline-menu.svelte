...
<script lang="ts">
  ...
  // PERF: height is only needed when position is top-* (to flip the menu above
  // the button). bind:clientHeight in the original menu.svelte reads
  // element.clientHeight synchronously inside every Svelte effect flush — with
  // 10k rows mounting simultaneously this was 4,531 forced synchronous layouts
  // (8.6% of active CPU). We replace it with a ResizeObserver that only
  // activates for top-* positions (the only case where height is consumed).
  let height = $state(0);
  let anchorElement = $state<HTMLElement | null>(null);

  $effect(() => {
    if (!menuElement || (position !== 'top-right' && position !== 'top-left'))
      return;
