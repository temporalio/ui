...
type ClientRunner =
  typeof import('./group-details-row-client-test-runner-entry');
...
export async function getGroupDetailsRowClientTestRunner(): Promise<ClientRunner> {
  ...
  try {
    ...
    viteServer = await createServer({
      ...
      resolve: {
        ...
        alias: [
          {
            find: '$lib/components/event/event-details-full.svelte',
            replacement: path.resolve(
              projectRoot,
              'src/lib/components/lines-and-dots/timeline-graph/group-details-row-event-details-test-double.svelte',
            ),
          },
          {
            find: './graph-widget.svelte',
            replacement: path.resolve(
              projectRoot,
              'src/lib/components/lines-and-dots/timeline-graph/group-details-row-graph-widget-test-double.svelte',
            ),
          },
          {
            find: '/@vite/client',
    ...
    runner = (await clientEnvironment.runner.import(
      '/src/lib/components/lines-and-dots/timeline-graph/group-details-row-client-test-runner-entry.ts',
    )) as ClientRunner;
    runner.initialize();
    return runner;
  } catch (error) {
    await closeGroupDetailsRowClientTestRunner();
    throw error;
  }
}

export async function closeGroupDetailsRowClientTestRunner() {
  runner = undefined;
