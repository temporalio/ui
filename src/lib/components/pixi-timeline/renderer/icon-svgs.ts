/**
 * Lightweight SVG icon helpers — no Pixi.js dependency.
 * Shared by both the Pixi renderer (via icon-textures.ts) and Svelte panel components.
 */
...
export const ICON_DEFS: Record<PixiIconName, IconDef> = {
  workflow: {
    paths: [
...
export function buildIconSvg(name: PixiIconName): string {
  const def = ICON_DEFS[name];
  let inner = '';
  for (const d of def.paths) inner += `<path d="${d}" fill="currentColor"/>`;
  if (def.circles) {
    for (const c of def.circles)
      inner += `<circle cx="${c.cx}" cy="${c.cy}" r="${c.r}" fill="currentColor"/>`;
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="100%" height="100%">${inner}</svg>`;
}

/** Returns an inline SVG string for the given pixiType, or null if unmapped. */
export function getEventIconSvg(eventType: string): string | null {
  const name = PIXI_TYPE_TO_ICON[eventType];
  return name ? buildIconSvg(name) : null;
}
