<script lang="ts">
  ...
  import EditViewModal from '$lib/components/standalone-nexus-operations/nexus-operations-summary-filter-bar/edit-view-modal.svelte';
  import SaveViewModal from '$lib/components/standalone-nexus-operations/nexus-operations-summary-filter-bar/save-view-modal.svelte';
  ...
  const maxViewsReached = $derived(
    $savedNexusQueries?.[namespace]?.length >= MAX_SAVED_NEXUS_QUERIES,
  );

  const namespaceSavedQueries = $derived(
    sortAlphabetically($savedNexusQueries?.[namespace] || [], (q) => q.name),
  );
  const systemQueryView = $derived(
    (query && systemNexusViews.find((q) => q.query === query)) ||
      (!query && DEFAULT_NEXUS_SYSTEM_VIEW),
  );
  ...
  onMount(() => {
    if (savedQueryParam) {
      ...
      if (!$savedNexusQueries[namespace]) $savedNexusQueries[namespace] = [];

      if (!maxViewsReached) {
        $savedNexusQueries[namespace] = [
          ...$savedNexusQueries[namespace],
          queryToSave,
        ];
      }
  ...
  $effect(() => {
    ...
    if (!query && activeQueryView?.id !== DEFAULT_NEXUS_SYSTEM_VIEW.id) {
      activeQueryView = DEFAULT_NEXUS_SYSTEM_VIEW;
      return;
    }
  ...
  const onCreateView = (view: SavedQuery) => {
    if (!$savedNexusQueries[namespace]) {
      $savedNexusQueries[namespace] = [];
    }

    $savedNexusQueries[namespace] = [...$savedNexusQueries[namespace], view];
    activeQueryView = view;
  };
  ...
  const onSaveView = (view: SavedQuery) => {
    ...
    if (view.id === activeQueryView?.id) {
      $savedNexusQueries[namespace] = $savedNexusQueries[namespace].map((q) =>
        q.id === view.id ? view : q,
      );
    } else {
      $savedNexusQueries[namespace] = [...$savedNexusQueries[namespace], view];
  ...
  const onDeleteView = (view: SavedQuery) => {
    $savedNexusQueries[namespace] = $savedNexusQueries[namespace].filter(
      (q) => q?.id !== view.id,
    );
    $nexusOperationFilters = [];
...
>
  ...
  <div class="space-y-2 p-1.5">
    <div class="pb-2 text-center">
      <div class="space-y-1">
        {#each systemNexusViews as view (view.id)}
          {@render queryButton({
            ...view,
    ...
    {#if $savedQueryNavOpen}
      <p
        ...
        in:slide
      >
        {translate('standalone-nexus-operations.custom-views')}
        {@render queryBadge({
          className: 'font-mono',
          content: `${namespaceSavedQueries.length}/${MAX_SAVED_NEXUS_QUERIES}`,
        })}
      </p>
    {/if}
