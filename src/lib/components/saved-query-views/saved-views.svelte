...
<div class="flex overflow-auto">
  <div
    class={merge(
      'relative h-[var(--panel-h)] h-auto max-h-[var(--panel-h)] min-h-[var(--panel-h)] w-[var(--panel-collapsed-w)] min-w-[var(--panel-collapsed-w)] max-w-[var(--panel-collapsed-w)] overflow-auto border border-r-0 border-primary bg-surface-primary text-primary shadow-sm transition-all duration-300 ease-in-out',
      $savedQueryNavOpen
  ...
  >
    <div
      class="flex items-center justify-center gap-2 border-b border-primary px-2 py-[.35rem] text-center lg:justify-start lg:py-[.47rem]"
    >
      <div
        class={merge(
      ...
      >
        ...
        <button
          class="hidden rounded-sm p-0.5 hover:bg-action-hover-overlay lg:inline-flex"
...
{#snippet queryBadge({
  ...
  <span
    class={merge(
      'right-2 top-2 hidden items-center rounded-full bg-surface-tertiary px-2 py-1 text-xs font-medium text-secondary lg:static lg:ml-auto lg:flex',
