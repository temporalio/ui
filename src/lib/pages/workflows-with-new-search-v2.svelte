<script lang="ts">
  ...
  import WorkflowsSummaryTableWithFilters from '$lib/components/workflow/workflows-summary-table-with-filters-v2.svelte';
  import WorkflowsSummaryRowWithFilters from '$lib/components/workflow/workflows-summary-row-with-filters-v2.svelte';
  ...
  const columns = [
    { path: 'select', width: 48, resize: false },
    { path: 'status', width: 128 },
    { path: 'workflowId' },
    { path: 'workflowType', width: 240 },
    { path: 'startTime', width: 240 },
    { path: 'endTime', width: 240 },
    { path: 'orderableList', width: 32, resize: false },
  ];
</script>
    ...
    {:else}
      <tr>
        <td
          ...
          style="grid-column: span {columns.length} / span {columns.length};"
        >
          {#if $loading}
            <Loading />
          {:else}
