...
The workflow catalog is a local-development page for browsing and running Temporal workflow examples. This document explains how it is configured. For the authoring workflow — adding, running, and promoting examples — install the `workflow-catalog` skill (`../../../agent-skills/skills/workflow-catalog/SKILL.md`) or read it directly.
...
**Namespace comes from where you are.** In the browser, it is the namespace in the URL: `/namespaces/staging/workflow-catalog` targets `staging`. For the worker, it comes from `TEMPORAL_NAMESPACE`. These are separate settings, so point them at the same namespace — otherwise the catalog shows examples whose readiness reports no worker polling, because the worker is polling somewhere else.
...
| `.env.workflow-catalog.local`         | no (gitignored) | Your overrides and all credentials                   |
| `.env.workflow-catalog.local.example` | yes             | Commented template covering every credential mode    |

Copy the example file to `.env.workflow-catalog.local` and uncomment what you need. Only the values that differ from the defaults belong there.
...
| `WORKFLOW_CATALOG_TARGET_ID` | Optional. Run one target instead of all of them.                    |
...
The browser reads generated artifacts rather than importing worker code. `catalog.generated.json`, `catalog.generated.ts`, and the local assemblies under `workflow-catalog.local/` are all generated: never edit them by hand.

```bash
pnpm workflow-catalog generate   # rebuild
pnpm workflow-catalog verify     # check for staleness and drift
```
