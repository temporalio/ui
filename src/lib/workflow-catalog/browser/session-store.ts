...
export type WorkflowCatalogSessionState =
  | 'execution-terminal'
  | 'launch-rejected'
  | 'launch-uncertain'
  | 'observation-paused'
...
export type WorkflowCatalogSession = {
  id: string;
  createdAt: number;
  exampleId: string;
  exampleTitle: string;
  ...
  state: WorkflowCatalogSessionState;
  snapshot?: JsonValue;
...
export type WorkflowCatalogSessionStoreValue = {
  sessions: readonly WorkflowCatalogSession[];
};

export type WorkflowCatalogEditorDraft = {
  inputEditor: string;
  startOptionsEditor: string;
};

export type WorkflowCatalogSessionStartRequest = {
...
export type WorkflowCatalogSessionStore =
  Readable<WorkflowCatalogSessionStoreValue> & {
    start: (
      request: WorkflowCatalogSessionStartRequest,
    ) => Promise<WorkflowCatalogSession | undefined>;
    getDraft: (exampleId: string) => WorkflowCatalogEditorDraft | undefined;
    setDraft: (exampleId: string, draft: WorkflowCatalogEditorDraft) => void;
    getSessions: () => readonly WorkflowCatalogSession[];
    latest: (
      exampleId: string,
      target: BrowserWorkflowCatalogDescriptor['execution'] | LaunchTarget,
    ) => WorkflowCatalogSession | undefined;
...
type SessionStoreOptions = {
  createId?: () => string;
  now?: () => number;
  startAllowed?: (descriptor: BrowserWorkflowCatalogDescriptor) => boolean;
};
...
export const workflowCatalogTargetFingerprint = targetFingerprint;
...
/**
 * Keeps workflow-catalog launches alive for one catalog refresh. It deliberately
 * has no persistence: callers create a fresh store when the catalog host refreshes.
 */
export const createWorkflowCatalogSessionStore = (
  host: WorkbenchHost,
  {
    createId,
    now = Date.now,
  ...
  let disposed = false;
  let sessions: WorkflowCatalogSession[] = [];
  const drafts = new Map<string, WorkflowCatalogEditorDraft>();
  const launchControllers = new Map<string, AbortController>();
  const observationControllers = new Map<string, AbortController>();
  const store = writable<WorkflowCatalogSessionStoreValue>({ sessions });
  ...
  const update = (
    attemptId: string,
    change: (session: WorkflowCatalogSession) => WorkflowCatalogSession,
  ) => {
    if (disposed) return;
    let changed = false;
  ...
  const start: WorkflowCatalogSessionStore['start'] = async ({
    descriptor,
    input,
    startOptions,
  }) => {
    ...
    const session: WorkflowCatalogSession = {
