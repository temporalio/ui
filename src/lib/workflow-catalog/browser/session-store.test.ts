...
describe('Workflow catalog session store', () => {
  ...
  it('keeps editor drafts scoped to each example', () => {
    const store = createWorkflowCatalogSessionStore(host(vi.fn()));
  ...
  it('observes an accepted attempt after its last subscriber unsubscribes', async () => {
    ...
    const store = createWorkflowCatalogSessionStore(catalogHost, {
      createId: (() => {
