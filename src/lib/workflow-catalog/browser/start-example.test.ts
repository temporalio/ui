...
const run = (
  ...
  currentDescriptor: BrowserWorkflowCatalogDescriptor = descriptor,
  ...
  startFromEditors({
    ...
    sessionStore: createWorkflowCatalogSessionStore(catalogHost, {
      createId: () => 'attempt-1',
    }),
  });

describe('workflow catalog example start', () => {
  ...
  it('does not dispatch invalid JSON, input, or start options', async () => {
    const catalogHost = host();
    const sessionStore = createWorkflowCatalogSessionStore(catalogHost);
