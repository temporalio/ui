...
let isCurrentCatalogDetailRequest: (request: {
  ...
  currentEpoch: number;
  requestDescriptor: BrowserWorkflowCatalogDescriptor;
  requestEpoch: number;
}) => boolean;
...
describe('WorkflowCatalogDetail', () => {
  ...
  it('rejects an old pre-run result after the descriptor changes', () => {
    ...
    } satisfies BrowserWorkflowCatalogDescriptor;
  ...
  it('renders prior sessions in a bounded native table below configuration', async () => {
    ...
    const sessionStore = createWorkflowCatalogSessionStore(sessionHost, {
      createId: () => 'attempt-1',
      now: () => Date.now() - 60_000,
    });
  ...
  it('uses human-readable names for every execution kind', () => {
    const sessionStore = createWorkflowCatalogSessionStore(host);
    const activityDescriptor: BrowserWorkflowCatalogDescriptor = {
      ...descriptor,
    ...
    const nexusDescriptor: BrowserWorkflowCatalogDescriptor = {
      ...descriptor,
      capabilityTags: [],
      execution: {
