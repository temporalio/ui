import type { BrowserWorkflowCatalogExecution, JsonValue } from './types';
...
export type LaunchReference = {
  exampleId: string;
  kind: BrowserWorkflowCatalogExecution['kind'];
  attempt: AttemptIdentity;
  target: LaunchTarget;
  runId?: string;
};
...
export type WorkflowCatalogStartPolicy = {
  disableWriteActions: boolean;
  workflowStartsDisabled: boolean;
  namespaceWriteDisabled: (namespace: string) => boolean;
};

export const workflowCatalogStartAllowed = (
  descriptor: {
    execution: Pick<BrowserWorkflowCatalogExecution, 'kind' | 'namespace'>;
  },
  policy: WorkflowCatalogStartPolicy,
): boolean =>
