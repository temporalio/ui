...
import { createWorkflowCatalogExecutionLogger } from './workflow-execution-logger.js';
...
describe('createWorkflowCatalogExecutionLogger', () => {
  it('emits a whitelisted workflow execution event when a workflow starts', () => {
    const logger = createWorkflowCatalogExecutionLogger(createFallbackLogger());
    const output = vi
      .spyOn(console, 'info')
  ...
  it('delegates ordinary SDK logs to the injected fallback logger', () => {
    ...
    const logger = createWorkflowCatalogExecutionLogger(fallbackLogger);
    const output = vi
      .spyOn(console, 'info')
      .mockImplementation(() => undefined);
