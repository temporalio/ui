...
import type { WorkflowCatalogWorkerBinding } from './registry';

const binding = (
  targetId: string,
  namespace: string,
  taskQueue: string,
  executions: WorkflowCatalogWorkerBinding['examples'][number]['execution'][],
): WorkflowCatalogWorkerBinding => ({
  target: {
    id: targetId,
    namespace,
    taskQueue,
...
const nexusExecution = (
  endpoint: string,
): WorkflowCatalogWorkerBinding['examples'][number]['execution'] => ({
