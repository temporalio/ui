...
export type WorkflowCatalogBannerTarget = {
  id: string;
  namespace: string;
  taskQueue: string;
};
...
export const formatWorkflowCatalogBanner = ({
  targets,
  origin = 'http://localhost:3000',
  color = false,
}: {
  targets: readonly WorkflowCatalogBannerTarget[];
  origin?: string;
  color?: boolean;
}): string => {
  const namespaces = [...new Set(targets.map(({ namespace }) => namespace))];
  const lines: BannerLine[] = [
    { text: 'WORKFLOW CATALOG READY', style: `${BOLD}${GREEN}` },
    { text: '', style: '' },
    ...namespaces.map((namespace) => ({
      text: `${origin}/namespaces/${namespace}/workflow-catalog`,
      style: CYAN,
    })),
    { text: '', style: '' },
