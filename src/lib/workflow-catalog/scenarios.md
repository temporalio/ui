# Workflow catalog configuration scenarios

Every supported configuration, how to reproduce it, and what has been verified. See [Workflow catalog configuration](./README.md) for the model these scenarios exercise.
...
Committed defaults, built-in dev server, no `.env.workflow-catalog.local`.
...
```bash
pnpm dev                            # terminal 1
pnpm workflow-catalog worker        # terminal 2
```
...
Set `TEMPORAL_NAMESPACE` in `.env.workflow-catalog.local`. Every target moves to that namespace and keeps its registered task queue. Browse the matching namespace in the UI.
...
A worker or command killed mid-write can leave `.workflow-catalog.lock` and a `.workflow-catalog.transaction-*` directory. A later command then reports that another mutation is in progress. Deleting the lock, the journal, and the transaction directory clears it. These paths are gitignored.

**Verified** as an observed failure and recovery.

### Killing the worker leaves child processes

Stopping `pnpm workflow-catalog worker` with a signal can leave the underlying Node process polling. `pnpm workflow-catalog dev` stops its whole process group correctly; the standalone worker command does not.
