...
const generateWorkflowExampleWithInputSchema = (
  ...
  return generateWorkflowCatalog(registry);
};

describe('generateWorkflowCatalog', () => {
  ...
  it('generates a workflow registration into browser-safe intent and a resolved Node binding', () => {
    ...
    expect(generated.nodeBindings).toEqual([
      {
        target: {
          id: 'catalog',
          namespace: 'default',
  ...
  it('generates a standalone activity with locked browser intent and an executable Node binding', () => {
    ...
    expect(generated.nodeBindings[0]?.examples).toEqual([
      {
        id: 'standalone-greeting',
        execution: {
          kind: 'standalone-activity',
    ...
    expect(generated.nodeBindings[0]?.activities).toEqual({
      greetingActivity: activity,
    });
  });

  it('generates a standalone Nexus operation with locked browser intent and an executable Node binding', () => {
    ...
    expect(generated.nodeBindings[0]?.activities).toEqual({});
    expect(generated.nodeBindings[0]?.nexusServices).toEqual([handler]);
  });
  ...
  it('reuses an identical Nexus service handler across operations', () => {
    ...
    expect(
      generateWorkflowCatalog(registry).nodeBindings[0]?.nexusServices,
    ).toEqual([handler]);
  });
  ...
  it('rejects conflicting Nexus service handlers before worker creation', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog Nexus service "GreetingService" has conflicting handlers on target "nexus-handler"',
    );
  });
  ...
  it('rejects a Nexus service name that differs from its handler definition', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog Nexus service "OtherService" for example "mismatched-service" does not match handler service "GreetingService"',
    );
  });
  ...
  it('rejects a Nexus operation missing from its handler definition', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog Nexus operation "missing" for example "missing-operation" is not defined by service "GreetingService"',
    );
  });
  ...
  it('fails when two registrations use the same example ID', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Duplicate workflow catalog example id "greeting"',
    );
  });
  ...
  it('fails when an example references an unknown target ID', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Unknown workflow catalog target id "missing-target" for example "orphaned-activity"',
    );
  });
  ...
  it('fails when browser metadata cannot be serialized as JSON', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog descriptor "cyclic-input" is not JSON-serializable',
    );
  });
  ...
  it('fails when two targets use the same target ID', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Duplicate workflow catalog target id "catalog"',
    );
  });
  ...
  it('fails when browser metadata contains a non-finite JSON number', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog descriptor "non-finite-input" is not JSON-serializable',
    );
  });
  ...
  it('fails when browser metadata contains a sparse JSON array', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog descriptor "sparse-input" is not JSON-serializable',
    );
  });
  ...
  it('fails when browser metadata contains an executable property accessor', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog descriptor "getter-input" is not JSON-serializable',
    );
  });
  ...
  it('fails without executing browser metadata proxy traps', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog descriptor "proxy-input" is not JSON-serializable',
    );
  ...
  it('fails without executing top-level registration metadata accessors', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog example registrations must use enumerable data properties',
    );
  ...
  it('fails without executing nested execution accessors', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog execution registrations must use enumerable data properties',
    );
    expect(accessorCalls).toBe(0);
  });
  ...
  it('rejects unknown top-level registration fields before browser projection', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog example "unknown-metadata" contains unknown fields',
    );
  });
  ...
  it('rejects cross-kind fields on workflow executions', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog workflow execution for example "workflow-with-activity-id" contains unknown fields',
    );
  });
  ...
  it('rejects invalid workflow execution runtime field types', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog workflow execution for example "workflow-with-numeric-type" has invalid field types',
    );
  });
  ...
  it('rejects cross-kind fields on standalone activity executions', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog standalone activity execution for example "activity-with-workflow-type" contains unknown fields',
    );
  });
  ...
  it('rejects invalid standalone activity execution runtime field types', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog standalone activity execution for example "activity-with-string-implementation" has invalid field types',
    );
  });
  ...
  it('rejects boolean standalone activity timeouts', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog standalone activity execution for example "activity-with-boolean-timeouts" has invalid field types',
    );
  });
  ...
  it('rejects boolean standalone activity policies', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog standalone activity execution for example "activity-with-boolean-policies" has invalid field types',
    );
  });
  ...
  it('rejects cross-kind fields on standalone Nexus executions', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog standalone Nexus execution for example "nexus-with-activity-id" contains unknown fields',
    );
  });
  ...
  it('rejects invalid standalone Nexus execution runtime field types', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog standalone Nexus execution for example "nexus-with-array-policies" has invalid field types',
    );
  });
  ...
  it('rejects boolean standalone Nexus policies', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog standalone Nexus execution for example "nexus-with-boolean-policies" has invalid field types',
    );
  });
  ...
  it('requires a non-empty packaged workflow path on every target', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog target "catalog" has an invalid packaged workflowsPath',
    );
  });
  ...
  it('rejects missing or invalid known top-level registration fields', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog example "missing-title" has invalid metadata fields',
    );
  });
  ...
  it('rejects unknown execution discriminants before variant validation', () => {
    ...
    expect(() => generateWorkflowCatalog(registry)).toThrowError(
      'Workflow catalog example "unknown-execution-kind" has unknown execution kind "query"',
    );
  });
    ...
    const invalidTargets = [
      {
        ...
        error: 'Workflow catalog target "catalog" contains unknown fields',
      },
      {
        target: {
          id: 'catalog',
        ...
        error:
          'Workflow catalog target "catalog" has invalid identity or routing fields',
      },
      {
        target: {
          id: 'catalog',
        ...
        error: 'Workflow catalog target "catalog" has invalid workflow exports',
      },
    ];
