...
type BrowserWorkflowCatalogDescriptorWithoutSource = Omit<
  BrowserWorkflowCatalogDescriptor,
  'source'
>;
...
type ExampleMetadata = BrowserWorkflowCatalogMetadata & {
  targetId: string;
};
...
export type WorkflowCatalogExampleRegistration =
...
export type WorkflowCatalogRegistry = {
  ...
  readonly targets: readonly WorkflowCatalogTarget[];
  readonly examples: readonly WorkflowCatalogExampleRegistration[];
};

export type WorkflowCatalogNodeBinding = {
  target: WorkflowCatalogTarget;
  examples: {
    id: string;
    execution: WorkflowCatalogExampleRegistration['execution'];
  }[];
...
export const createWorkflowCatalogRegistry = (): WorkflowCatalogRegistry => {
  const targets: WorkflowCatalogTarget[] = [];
  const examples: WorkflowCatalogExampleRegistration[] = [];
...
export const generateWorkflowCatalog = (registry: WorkflowCatalogRegistry) => {
  for (const target of registry.targets) {
    if (!hasOnlyEnumerableDataProperties(target)) {
      throw new Error(
        'Workflow catalog target registrations must use enumerable data properties',
      );
    }

    if (!hasOnlyKnownFields(target, targetFields)) {
      throw new Error(
        `Workflow catalog target "${target.id}" contains unknown fields`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Workflow catalog target "${target.id}" has invalid identity or routing fields`,
      );
    }

    if (!isNonEmptyString(target.workflowsPath)) {
      throw new Error(
        `Workflow catalog target "${target.id}" has an invalid packaged workflowsPath`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        'Workflow catalog workflow exports must use enumerable data properties',
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Workflow catalog target "${target.id}" has invalid workflow exports`,
      );
    }
  }
  ...
  for (const example of registry.examples) {
    ...
    if (!hasOnlyKnownFields(example, exampleFields)) {
      throw new Error(
        `Workflow catalog example "${example.id}" contains unknown fields`,
      );
    }
    ...
    if (!executionKinds.has(example.execution.kind)) {
      throw new Error(
        `Workflow catalog example "${example.id}" has unknown execution kind "${String(example.execution.kind)}"`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Workflow catalog workflow execution for example "${example.id}" contains unknown fields`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Workflow catalog standalone activity execution for example "${example.id}" contains unknown fields`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Workflow catalog standalone Nexus execution for example "${example.id}" contains unknown fields`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Workflow catalog standalone Nexus execution for example "${example.id}" has invalid field types`,
      );
    }
    ...
    if (example.execution.kind === 'standalone-nexus-operation') {
      ...
      if (example.execution.service !== definition.name) {
        throw new Error(
          `Workflow catalog Nexus service "${example.execution.service}" for example "${example.id}" does not match handler service "${definition.name}"`,
        );
      }

      if (!Object.hasOwn(definition.operations, example.execution.operation)) {
        throw new Error(
          `Workflow catalog Nexus operation "${example.execution.operation}" for example "${example.id}" is not defined by service "${definition.name}"`,
        );
      }
    }
    ...
    if (
      ...
      throw new Error(
        `Workflow catalog standalone activity execution for example "${example.id}" has invalid field types`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Workflow catalog workflow execution for example "${example.id}" has invalid field types`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        'Workflow catalog activity registrations must use enumerable data properties',
      );
    }
  }
  ...
  for (const { id } of registry.targets) {
    if (targetIds.has(id)) {
      throw new Error(`Duplicate workflow catalog target id "${id}"`);
    }
  ...
  for (const { execution, id, targetId } of registry.examples) {
    if (exampleIds.has(id)) {
      throw new Error(`Duplicate workflow catalog example id "${id}"`);
    }
    ...
    if (!target) {
      throw new Error(
        `Unknown workflow catalog target id "${targetId}" for example "${id}"`,
      );
    }
  ...
  const nodeBindings: WorkflowCatalogNodeBinding[] = registry.targets
    .filter((target) =>
  ...
  const browserDescriptors: BrowserWorkflowCatalogDescriptorWithoutSource[] =
  ...
  for (const descriptor of browserDescriptors) {
    if (!isJsonSerializable(descriptor)) {
      throw new Error(
        `Workflow catalog descriptor "${descriptor.id}" is not JSON-serializable`,
      );
    }
  }
  ...
  return { browserDescriptors: browserSnapshots, nodeBindings };
};
