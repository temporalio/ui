...
export type WorkflowCatalogAuthoringTarget = {
...
export type WorkflowCatalogAuthoringOutput = {
  consumers: readonly ('authoring' | 'browser' | 'worker')[];
  path: string;
};

export type WorkflowCatalogAuthoringGraph = {
  boundaries: {
    browserPaths: readonly string[];
    packageJsonPath: string;
    workerPaths: readonly string[];
  };
  outputs: readonly WorkflowCatalogAuthoringOutput[];
  sources: readonly WorkflowCatalogAuthoringSource[];
  targets: readonly WorkflowCatalogAuthoringTarget[];
};

export type WorkflowCatalogComposedExample = {
  id: string;
...
export type WorkflowCatalogAssemblyExample = WorkflowCatalogComposedExample & {
  definitionImportPath: string;
  workflowExport?: string;
  workflowImportPath?: string;
};

export type WorkflowCatalogAuthoringAdapter = {
  generatedPaths: readonly string[];
  graph: WorkflowCatalogAuthoringGraph;
  generate: () =>
    | readonly WorkflowCatalogAuthoringArtifact[]
    | Promise<readonly WorkflowCatalogAuthoringArtifact[]>;
  localExamplesPath: string;
  loadExamples: () =>
    | readonly WorkflowCatalogComposedExample[]
    | Promise<readonly WorkflowCatalogComposedExample[]>;
...
const artifactContent = (artifact: WorkflowCatalogAuthoringArtifact) =>
...
const assertExplicitRelativePath = (path: string) => {
  if (
    ...
    throw new Error('Workflow catalog paths must be explicit relative paths');
  }
};
...
export const defineWorkflowCatalogArtifact = (
  path: string,
  content: string | Uint8Array,
): WorkflowCatalogAuthoringArtifact => {
  assertExplicitRelativePath(path);
  return { content, path };
};

export const deleteWorkflowCatalogArtifact = (
  path: string,
): WorkflowCatalogAuthoringArtifact => {
  assertExplicitRelativePath(path);
...
export const composeWorkflowCatalogArtifacts = (
  ...groups: readonly (readonly WorkflowCatalogAuthoringArtifact[])[]
) => {
  ...
  for (const artifact of artifacts) {
    ...
    if (paths.has(artifact.path)) {
      throw new Error(`Duplicate workflow catalog artifact: ${artifact.path}`);
    }
...
export const validateWorkflowCatalogAuthoringGraph = (
  ...
  const uniqueById = <Value extends { id: string }>(
    ...
    for (const value of values) {
      ...
      if (byId.has(value.id)) {
        throw new Error(`Duplicate workflow catalog ${kind} id: ${value.id}`);
      }
  ...
  const outputsByPath = new Map<string, WorkflowCatalogAuthoringOutput>();
  for (const output of graph.outputs) {
    assertExplicitRelativePath(output.path);
    if (outputPaths.has(output.path)) {
      throw new Error(`Duplicate workflow catalog output path: ${output.path}`);
    }
    if (output.consumers.length === 0) {
      throw new Error(
        `Workflow catalog output has no consumers: ${output.path}`,
      );
    }
  ...
  for (const source of graph.sources) {
    ...
    if (!source.label.trim()) {
      throw new Error(
        `Workflow catalog source label is required: ${source.id}`,
      );
    }
    if (!outputPaths.has(source.registrationOutputPath)) {
      throw new Error(
        `Workflow catalog source output is undeclared: ${source.registrationOutputPath}`,
      );
    }
  }
  for (const target of graph.targets) {
    ...
    if (!sources.has(target.sourceId)) {
      throw new Error(
        `Workflow catalog target references an unknown source: ${target.sourceId}`,
      );
    }
    if (!target.defaultNamespace.trim() || !target.defaultTaskQueue.trim()) {
      throw new Error(
        `Workflow catalog target must declare a default namespace and task queue: ${target.id}`,
      );
    }
    if (!outputPaths.has(target.workflowsModulePath)) {
      throw new Error(
        `Workflow catalog workflows output is undeclared: ${target.workflowsModulePath}`,
      );
    }
  }
  for (const path of [
  ...
  for (const path of graph.boundaries.browserPaths) {
    ...
    if (output && !output.consumers.includes('browser')) {
      throw new Error(
        `Workflow catalog browser boundary must declare a browser consumer: ${path}`,
      );
    }
  }
  for (const path of graph.boundaries.workerPaths) {
    if (browserPaths.has(path)) {
      throw new Error(
        `Workflow catalog boundary cannot be both browser and worker: ${path}`,
      );
    }
    const output = outputsByPath.get(path);
    if (output && !output.consumers.includes('worker')) {
      throw new Error(
        `Workflow catalog worker boundary must declare a worker consumer: ${path}`,
      );
    }
  }
  ...
  for (const example of examples) {
    ...
    if (!source || !target || target.sourceId !== source.id) {
      throw new Error(
        `Workflow catalog example has an invalid source/target composition: ${example.id}`,
      );
    }
...
export const renderWorkflowCatalogSourceAssembly = ({
  examples,
  source,
  target,
}: {
  examples: readonly WorkflowCatalogAssemblyExample[];
  source: WorkflowCatalogAuthoringSource;
  target: WorkflowCatalogAuthoringTarget;
}) => {
  if (target.sourceId !== source.id) {
    throw new Error(
      'Workflow catalog source assembly target has the wrong source',
    );
  }
  ...
  for (const example of ordered) {
    if (example.sourceId !== source.id || example.targetId !== target.id) {
      throw new Error(
        `Workflow catalog assembly example is assigned to another source or target: ${example.id}`,
      );
    }
  ...
  const imports = ordered.map(
    (example, index) =>
      `import { workflowCatalogExample as example${index} } from '${moduleSpecifier(source.registrationOutputPath, example.definitionImportPath)}';`,
  );
  ...
  const registration = [
    ...
    ...imports,
    `import type { WorkflowCatalogRegistrationSource } from '${moduleSpecifier(source.registrationOutputPath, source.registrationTypePath)}';`,
  ...
  return composeWorkflowCatalogArtifacts([
    defineWorkflowCatalogArtifact(source.registrationOutputPath, registration),
...
export const createWorkflowCatalogAuthoring = (
  adapter: WorkflowCatalogAuthoringAdapter,
) => {
  validateWorkflowCatalogAuthoringGraph(adapter.graph, []);
  ...
  for (const path of [
    ...
    if (adapter.generatedPaths.includes(path) && !declaredOutputs.has(path)) {
      throw new Error(
        `Workflow catalog generated output is undeclared: ${path}`,
      );
    }
  }
  ...
  const safeAbsolutePath = async (path: string) => {
    ...
    for (const [index, segment] of segments.entries()) {
      ...
      if (metadata.isSymbolicLink()) {
        throw new Error(
          `Workflow catalog paths cannot traverse symbolic links: ${path}`,
        );
      }
      if (index < segments.length - 1 && !metadata.isDirectory()) {
        throw new Error(
          `Workflow catalog path ancestor must be a directory: ${path}`,
        );
      }
      current = candidate;
    }
  ...
  const ensureSafeDirectory = async (path: string) => {
    ...
    for (const segment of path.split('/')) {
      ...
      if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
        throw new Error(
          `Workflow catalog paths cannot traverse symbolic links or special files: ${path}`,
        );
      }
    }
    return current;
  ...
  const writeArtifacts = async (
    artifacts: readonly WorkflowCatalogAuthoringArtifact[],
    transactionPath: string,
  ) => {
    ...
    const staged = await Promise.all(
      artifacts.map(async (artifact, index) => {
        ...
        if (!content)
          throw new Error('Workflow catalog artifact has no content');
    ...
    for (const { artifact, stagedPath } of staged) {
      ...
      if (!stagedPath)
        throw new Error('Workflow catalog artifact was not staged');
  ...
  const renderGeneratedArtifacts = async () => {
    ...
    for (const artifact of artifacts) {
      ...
      if (!allowedPaths.has(artifact.path)) {
        throw new Error(
          `Adapter returned an undeclared workflow catalog output: ${artifact.path}`,
        );
      }
    }
    return artifacts;
    ...
    return Promise.all(
      [...new Set(paths)].map(async (path, index): Promise<FileSnapshot> => {
        ...
        if (!metadata.isFile() || metadata.isSymbolicLink()) {
          throw new Error(
            `Workflow catalog transaction path must be a regular file: ${path}`,
          );
        }
  ...
  const journalRelativePath = '.workflow-catalog.journal.json';
  const writeJournal = async (journal: TransactionJournal) => {
    const temporaryPath = `.workflow-catalog.journal.${journal.id}.tmp`;
    await writeFile(
  ...
  const readJournal = async (): Promise<TransactionJournal | undefined> => {
    ...
    if (!metadata.isFile() || metadata.isSymbolicLink()) {
      throw new Error(
        'Workflow catalog transaction journal must be a regular file',
      );
    }
    ...
    if (
      ...
      journal.transactionPath !==
        `.workflow-catalog.transaction-${journal.id}` ||
      !Array.isArray(journal.snapshots)
    ) {
      throw new Error('Workflow catalog transaction journal is invalid');
    }
  ...
  const recoverJournal = async () => {
    ...
    if (rollbackErrors.length > 0) {
      throw new AggregateError(
        rollbackErrors,
        'Workflow catalog crash recovery was incomplete',
      );
    }
  ...
  const withJournaledFiles = async <Result>(
    ...
    const id = randomUUID();
    const transactionPath = `.workflow-catalog.transaction-${id}`;
    ...
    try {
      ...
      try {
        ...
        throw new AggregateError(
          [error, rollbackError],
          'Workflow catalog transaction failed and rollback was incomplete',
        );
      }
      throw error;
    } finally {
  ...
  const verifyUnlocked = async () => {
    ...
    validateWorkflowCatalogAuthoringGraph(
      adapter.graph,
      await adapter.loadExamples(),
    );
  };
  const withLock = async <Result>(action: () => Promise<Result>) => {
    const lockRelativePath = '.workflow-catalog.lock';
    const recoveryLockRelativePath = '.workflow-catalog.lock.recovery';
    ...
    const acquireLock = async () => {
      ...
      const acquireRecoveryLock = async () => {
        ...
        try {
          for (;;) {
            ...
            if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
              throw new Error(
                'Workflow catalog recovery lock must be a real directory',
              );
            }
            ...
            if (!observedToken) {
              throw new Error(
                'Workflow catalog recovery lock metadata is invalid',
              );
            }
      ...
      try {
        ...
        if (!metadata.isFile() || metadata.isSymbolicLink()) {
          throw new Error(
            'Workflow catalog mutation lock must be a regular file',
          );
        }
        ...
        const stalePath = `.workflow-catalog.lock.stale-${randomUUID()}`;
  ...
  const validateSelfContainedExample = async ({
    ...
    const validateDirectory = async (directory: string): Promise<string[]> => {
      ...
      for (const entry of entries) {
        ...
        if (!entry.isFile()) {
          throw new Error(
            `Workflow catalog example sources must be regular files: ${relative(root, path)}`,
          );
        }
        files.push(path);
      }
...
export const runWorkflowCatalogCli = async ({
  ...
  authoring: ReturnType<typeof createWorkflowCatalogAuthoring>;
  ...
  if (command === 'scaffold' && argv.length === 2) {
    ...
    io.writeOutput(
      `Created local workflow catalog example "${argv[1]}". Edit its files, then run "pnpm workflow-catalog dev".`,
    );
    return;
  }
  if (command === 'generate' && argv.length === 1) {
    await authoring.generate();
    io.writeOutput('Workflow catalog artifacts generated.');
    return;
  }
...
export const loadWorkflowCatalogLocalArtifactDescriptors = async ({
  artifactPath,
  rootDirectory,
  validateDescriptor = () => true,
}: Pick<
  WorkflowCatalogLocalArtifactVitePluginOptions,
...
> & { rootDirectory: string }) => {
  ...
  if (!Array.isArray(artifact.descriptors)) {
    throw new Error(
      `Workflow catalog artifact "${artifactPath}" must contain a descriptors array`,
    );
  }
  if (!artifact.descriptors.every(validateDescriptor)) {
    throw new Error(
      `Workflow catalog artifact "${artifactPath}" contains an invalid local descriptor`,
    );
  }
  return artifact.descriptors;
};

export const createWorkflowCatalogLocalArtifactVitePlugin = ({
  artifactPath,
  regenerate,
  sourcePaths = [],
  sourceRootPaths = [],
  validateDescriptor = () => true,
  virtualModuleId = 'virtual:workflow-catalog-local',
}: WorkflowCatalogLocalArtifactVitePluginOptions) => {
  ...
  return {
    ...
    async load(id: string) {
      ...
      const descriptors = await loadWorkflowCatalogLocalArtifactDescriptors({
        artifactPath,
        rootDirectory,
        validateDescriptor,
      });
