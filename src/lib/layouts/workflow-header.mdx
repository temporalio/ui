import { Meta } from '@storybook/blocks';
import { Markdown } from '@storybook/blocks';

import WorkflowHeaderStories from './workflow-header.stories.svelte';

<Meta of={WorkflowHeaderStories} />

# WorkflowHeader additionalDetails Snippet

The `WorkflowHeader` component supports an `additionalDetails` snippet parameter for injecting custom content into the workflow header section.

## Snippet Parameter

```typescript
interface Props {
  additionalDetails?: Snippet;
}
```

The snippet is rendered between `WorkflowCurrentDetails` and `WorkflowDetails`:

```svelte
<WorkflowCurrentDetails />
{@render additionalDetails?.()}
<WorkflowDetails {workflow} next={workflowRelationships.next} />
```

## Usage

### With WorkflowHeader

```svelte
<script lang="ts">
  import WorkflowHeader from '$lib/layouts/workflow-header.svelte';
</script>

<WorkflowHeader {additionalDetails} />

{#snippet additionalDetails()}
  <!-- Custom content goes here -->
{/snippet}
```

### With WorkflowRunLayout

```svelte
<script lang="ts">
  import WorkflowRunLayout from '$lib/layouts/workflow-run-layout.svelte';
</script>

<WorkflowRunLayout {additionalDetails}>
  <!-- Page content -->
</WorkflowRunLayout>

{#snippet additionalDetails()}
  <!-- Custom content goes here -->
{/snippet}
```

The `WorkflowRunLayout` automatically passes the snippet to `WorkflowHeader`.

## Example: Namespace Tags

```svelte
<script lang="ts">
  import { DetailListValue } from '$lib/components/detail-list';
  import Badge from '$lib/holocene/badge.svelte';
  import Icon from '$lib/holocene/icon/icon.svelte';

  const namespaceTags = [
    { key: 'Environment', value: 'Production' },
    { key: 'Application', value: 'Checkout' },
    { key: 'Team', value: 'Small' },
  ];
</script>

<WorkflowHeader {additionalDetails} />

{#snippet additionalDetails()}
  <div class="flex gap-2">
    <DetailListValue>
      <div class="flex flex-wrap gap-2">
        {#each namespaceTags as tag}
          <div
            class="inline-flex"
            role="img"
            aria-label="Tag: {tag.key} equals {tag.value}"
          >
            <Badge
              class="inline-flex items-center gap-1 rounded-r-none border-r-0 bg-blue-100 text-xs dark:bg-indigo-700 dark:text-white"
            >
              <Icon name="tag" class="h-3 w-3" />
              {tag.key}
            </Badge>
            <Badge
              class="inline-flex items-center rounded-l-none bg-blue-200 text-xs dark:bg-indigo-800 dark:text-white"
            >
              {tag.value}
            </Badge>
          </div>
        {/each}
      </div>
    </DetailListValue>
  </div>
{/snippet}
```
