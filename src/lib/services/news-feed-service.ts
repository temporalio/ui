...
export const USE_MOCK_NEWS_FEED: boolean = true;
...
export const fetchNewsFeed = async ({
  ...
  if (USE_MOCK_NEWS_FEED) {
    return toNewsFeedCache(MOCK_NEWS_FEED_RESPONSE, now());
  }

  const requestOptions: RequestInit = { credentials: 'omit' };
  if (cache) requestOptions.cache = cache;

  let feed: NewsFeedResponse = MOCK_NEWS_FEED_RESPONSE;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `News feed request failed with status ${response.status}`,
      );
    }

    feed = parseNewsFeedResponse(await response.json());
  } catch {
    feed = MOCK_NEWS_FEED_RESPONSE;
  }
